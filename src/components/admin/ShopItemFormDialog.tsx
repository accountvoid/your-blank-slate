...
export interface ShopItemFormValue {
  id?: string;
  item_key: string;
  name_ar: string;
  name_en: string;
...
interface Props {
  ...
  saving: boolean;
  onSubmit: (value: ShopItemFormValue) => Promise<void> | void;
}

export const ShopItemFormDialog = ({ open, onOpenChange, initial, saving, onSubmit }: Props) => {
  const [value, setValue] = useState<ShopItemFormValue>(empty);
  ...
  useEffect(() => {
    ...
    if (initial) {
      setValue({
        ...
        level_required: initial.level_required,
        rank_required: initial.rank_required,
        can_purchase: initial.can_purchase,
  ...
  const set = <K extends keyof ShopItemFormValue>(k: K, v: ShopItemFormValue[K]) =>
  ...
  const handleSubmit = async () => {
    ...
    if (value.price_gold < 0) return setError('Price cannot be negative');
    setError(null);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{initial ? 'Edit Shop Item' : 'Create Shop Item'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
          ...
          <Field label="Price (Gold)">
            <Input type="number" min={0} value={value.price_gold} onChange={(e) => set('price_gold', Math.max(0, Number(e.target.value) || 0))} />
          </Field>
          ...
          <Field label="Level Required">
            <Input type="number" min={0} value={value.level_required} onChange={(e) => set('level_required', Math.max(0, Number(e.target.value) || 0))} />
          </Field>
          <Field label="Rank Required">
            <Select value={value.rank_required} onValueChange={(v) => set('rank_required', v)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Effect Type">
            ...
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {EFFECT_TYPES.map((e) => <SelectItem key={e} value={e}>{e}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          ...
          <ToggleField label="Can Purchase" checked={value.can_purchase} onChange={(v) => set('can_purchase', v)} />
