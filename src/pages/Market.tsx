...
const Market = () => {
  ...
  // معالجة الشحن الفعلي ومحاكاة المزامنة مع gameState
  const handleFinalizeRecharge = () => {
    if (!transactionId.trim()) {
      toast({ title: "System Error", description: "Please enter a valid Transaction Hash / Bill ID.", variant: 'destructive' });
      return;
    }

    setIsSubmitting(true);
    toast({ title: "Verifying Request...", description: "Connecting to Terminal Sync Protocols." });

    setTimeout(() => {
      // شحن الذهب مباشرة داخل gameState
      gameState.gold += selectedOffer.gold;
      
      playPurchase();
      toast({ 
        title: "Synchronization Complete", 
        description: `Successfully added ${selectedOffer.gold.toLocaleString()} GOLD to your account.` 
      });
                ...
                <div className="space-y-6 animate-in zoom-in-95 duration-500">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button 
                            onClick={handleFinalizeRecharge}
                            disabled={isSubmitting || !transactionId.trim()}
                            className="flex-1 py-4 bg-white text-black font-black uppercase tracking-[0.4em] text-[10px] shadow-[0_0_30px_rgba(255,255,255,0.2)] active:scale-95 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
                        >
                            {isSubmitting ? (
                              <>
